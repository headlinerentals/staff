...
3. OS persistent app-data folder (automatic)
4. local default (`finance_hub.db` beside code, legacy fallback)

For deployment platforms, mount a persistent volume (disk) and set `HR_DATA_DIR` to that mount path.

Automatic backups:
- The app now creates rolling SQLite snapshots in `<db-folder>/auto_backups/`.
- Backups are auto-triggered after save/update/delete operations and periodically at startup.
- Manual invoice saves force an immediate backup snapshot.
- A live mirror file is also maintained at `finance_hub_live_backup.db` (same folder as the active DB when possible).
- In **Mobile & Team**, you can now select a specific backup timestamp and restore it immediately (`Restore Backup by Time`).
- Keep/timing can be tuned with env vars:
  - `HR_AUTO_BACKUP_MIN_SECONDS` (default `30`)
  - `HR_AUTO_BACKUP_KEEP` (default `120`)
- Legacy DB migration is now opt-in:
  - `HR_MIGRATE_LEGACY_DB=1` only if you intentionally want to copy `finance_hub.db` beside code into persistent storage.
- If the active DB is accidentally replaced with an empty DB, startup attempts auto-restore from the latest non-empty snapshot.

### 2.2) Cloud Backup for Streamlit Sleep/Redeploy (Recommended)

If your app host sleeps/redeploys (e.g., Streamlit Community Cloud), configure remote backup so data is restored on startup:

```bash
HR_REMOTE_BACKUP_REPO=your-github-username/your-private-repo
HR_REMOTE_BACKUP_TOKEN=github_pat_with_repo_access
HR_REMOTE_BACKUP_BRANCH=backup-storage
HR_REMOTE_BACKUP_FILE=backups/finance_hub_live_backup.db
HR_REMOTE_BACKUP_RESTORE_MODE=empty_or_smaller
HR_REMOTE_BACKUP_PULL_ON_INIT=1
```

Notes:
- `HR_REMOTE_BACKUP_TOKEN` should be stored in deployment secrets, not committed to git.
- Remote backup sync runs automatically when backup snapshots are created.
- Restore mode options:
  - `empty_only`: only restore when local DB is empty.
  - `empty_or_smaller`: restore when local DB is empty or has fewer rows than remote.
...
- Automatic backup snapshots:
  - Folder: `<db-folder>/auto_backups`
  - Tunables: `HR_AUTO_BACKUP_MIN_SECONDS`, `HR_AUTO_BACKUP_KEEP`
