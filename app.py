...
def render_reports(report_start_month: str) -> None:
    ...
    f1, f2, f3 = st.columns([1.7, 1.1, 1.6])
    selected_years = f1.multiselect(
        "Year Filter",
        options=year_values,
        default=year_values,
        key="reports_global_year_filter",
    )
    report_period = f2.selectbox(
        "Period Filter",
        options=["Daily", "Weekly", "Monthly", "Yearly"],
        index=2,
        key="reports_global_period_filter",
    )
    report_scope = f3.radio(
        "Scope Filter",
        options=["All Filtered Rows", "Selected Row"],
        horizontal=True,
        key="reports_global_scope_filter",
    )
    if selected_years:
        if not daily.empty:
    ...
    selected_end_ts: pd.Timestamp | None = None
    ...
    if (
        report_scope == "Selected Row"
        and selected_start_ts is not None
        and selected_end_ts is not None
        and pd.notna(selected_start_ts)
        and pd.notna(selected_end_ts)
    ):
