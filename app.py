...
from db import (
    ...
    purge_all_records,
    run_recurring_template_autopost,
    upsert_monthly_budget,
    upsert_recurring_expense_template,
    upcoming_invoices,
...
def inject_styles(theme_mode: str) -> None:
    ...
    <style>
        ...
        :root {
            ...
            --space-3: 24px;
            --space-4: 32px;
            --wattbot-bottom-offset: max(20px, env(safe-area-inset-bottom));
        ...
        @media (max-width: 860px) {
            :root {
                ...
                --wattbot-bottom-offset: calc(78px + env(safe-area-inset-bottom));
            }
            [data-testid="stAppViewBlockContainer"] {
...
def render_dashboard_storyboard(
    ...
    st.markdown(
        ...
        <div class="insight-strip">
            ...
            <div class="insight-chip"><b>Reporting Window:</b> {reporting_window_label(report_start_month)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_top_nav_bar(active_section: str) -> None:
    ...
    nav_items = [
        ("Today Home", "today", "Today Home"),
        ("Finance Hub", "finance", "Finance Hub"),
        ("Build Invoice", "invoices", "Build Invoice"),
        ("Client Retention", "retention", "Client Retention Automation"),
        ("Deposit Tracker", "deposit", "Deposit Due Tracker"),
        ("Supplier Re-Rental", "supplier", "Supplier Re-Rental"),
        ("Inventory", "inventory", "Inventory"),
        ("Import Legacy Data", "import", "Import Legacy Data"),
        ("Mobile & Team", "mobile", "Mobile & Team"),
    ]
    links = []
    for label, slug, section_name in nav_items:
        active_class = "active" if section_name == active else ""
        links.append(f"<a class='{active_class}' href='?sec={slug}'>{html.escape(label)}</a>")
    st.markdown(
        f"""
        <div class="hr-top-nav">
            {''.join(links)}
            <div class="meta">Current: {html.escape(active or 'Today Home')}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_invoice_profit_table() -> None:
    ...
    if pending.empty:
        ...
        if submit_payment:
            ...
            new_status = "paid_full" if new_paid >= invoice_total_value - 0.01 else "deposit_paid"
            set_invoice_payment_status(
            ...
            save_invoice(
                {
                    ...
                    or (
                        ...
                        and payment_map[edited_payment_label] == "deposit_paid"
                    ),
...
def render_finance_hub(
    ...
    if int(autopost_result.get("posted_count", 0)) > 0:
        clear_finance_caches()
        st.success(
            f"Auto-posted {int(autopost_result.get('posted_count', 0))} recurring expense(s) for {autopost_result.get('month', '')}."
        )
...
def render_dashboard(report_start_month: str, alert_window_days: int) -> None:
    ...
    invoice_level = apply_start_month(cached_invoice_level(), report_start_month)
    products = cached_product_profitability()
    expenses_view = apply_start_month(cached_expenses(), report_start_month)
    if not expenses_view.empty:
        expenses_view = expenses_view[
            expenses_view["expense_kind"].str.lower() != "summary_rollup"
        ]
        expenses_view = expenses_view[~inventory_purchase_mask(expenses_view["category"])]
    categories = (
    ...
    if monthly.empty:
        ...
        with c1:
            render_kpi(
                "Latest Month Revenue (Accounted)",
                money(float(latest["revenue"])),
                trend_values=revenue_trend,
            )
        with c2:
            render_kpi(
                "Latest Month Expenses",
                money(float(latest["total_expenses"])),
                trend_values=expense_trend,
            )
        with c3:
            render_kpi("Latest Month Net Profit", money(float(latest["net_profit"])), trend_values=net_trend)
        with c4:
            render_kpi(
                "After Adjustments",
        ...
        with p1:
            render_kpi(
                "Latest Month Cash Collected",
                money(float(latest.get("cash_collected", 0.0))),
        ...
        fig = px.line(
            ...
            title=f"Monthly Financial Trend ({reporting_window_label(report_start_month)})",
        )
        style_plotly(fig)
    ...
    if monthly.empty:
        ...
        month_options = monthly["month"].dropna().astype(str).drop_duplicates().tolist()
        ...
        close_expenses = expenses_view[expenses_view["month"] == selected_close_month].copy()
        close_invoices = invoice_level[invoice_level["month"] == selected_close_month].copy()
...
def render_invoices() -> None:
    ...
    def _render_saved_price_quotes_block() -> None:
        ...
        def _convert_selected_quote(selected_quote_id: int) -> None:
        ...
        with convert_tab:
            if st.button(
                ...
                try:
                    _convert_selected_quote(selected_quote_id)
                    st.success(
    ...
    st.caption(
        "Edit the table directly. Changes are captured automatically in the builder as you type."
    )
    items_edited = st.data_editor(
        ...
        column_config={
            ...
            "unit_price": st.column_config.NumberColumn(
                "Rental Price (JMD)", min_value=0.0, step=100.0
            ),
    ...
    items = normalize_invoice_items_df(items_edited)
    ...
    prior_items = normalize_invoice_items_df(
        st.session_state.get("invoice_items_editor_data", pd.DataFrame())
    )[
        ["item_name", "item_type", "quantity", "unit_price"]
    ].copy()
    if not persisted_items.reset_index(drop=True).equals(prior_items.reset_index(drop=True)):
        st.session_state["invoice_items_editor_data"] = persisted_items
    ...
    preview_b.metric("Deposit Due Now", money(float(paid_now_amount)))
    preview_c.metric("Balance Later", money(float(outstanding_amount)))
    ...
    def _render_last_saved_invoice_quick_actions() -> None:
        ...
        if latest_saved_id is not None:
            ...
            prepare_col, hint_col = st.columns([1.1, 2.4])
            if prepare_col.button(
                "Prepare Quick Download + Send",
                key=f"quick_invoice_prepare_{int(latest_saved_id)}",
            ):
                try:
                    ...
                    st.success("Quick actions prepared.")
                except Exception as exc:
                    st.error(f"Quick actions unavailable for latest invoice: {exc}")

            hint_col.caption(
                "Performance mode: files are generated only when you press prepare (faster while typing in table)."
            )
            ...
            if (
                ...
                st.caption("Click `Prepare Quick Download + Send` to generate PDF/PNG and send links.")
        else:
        ...
        if not invoice_number.strip():
            ...
            try:
                ...
                if document_type == "invoice" and order_status == "confirmed":
                    ...
                    st.info(
                        "Quote mode: this document does not impact Finance Hub or Inventory."
                    )
                st.caption(
...
def render_expenses() -> None:
    ...
    st.markdown("**Recurring Monthly Expense Templates**")
    st.caption(
        "Templates auto-post once per month into expenses. Ideal for ChatGPT, Google Ads, Shopify, subscriptions, and utilities."
    )
        ...
        if not template_name.strip():
            ...
            try:
                ...
                finance_audit_log(
                    ...
                    action_type="upsert",
                    notes=f"{template_name.strip()} | {template_category} | {money(float(template_amount))}",
                )
                clear_finance_caches()
    ...
    with st.form("expense_form", clear_on_submit=True):
        ...
        if expense_kind_label == "Transaction (invoice/day level)":
            link_options = confirmed_invoice_options
            link_label_text = "Link to Confirmed Invoice *"
            if not link_options:
                st.caption("No confirmed invoices with event date found yet.")
        else:
            link_options = ["Not linked to invoice"]
            link_label_text = "Link to Invoice"
        if not link_options:
            link_options = ["Not linked to invoice"]
        link_label = st.selectbox(link_label_text, link_options)
    ...
    if submit_expense:
        if amount <= 0:
            ...
            try:
                ...
                if (
                    str(category).strip().lower() == "re-rental"
                ...
                finance_audit_log(
                    ...
                    action_type="create",
                    notes=(
                        f"{category} | {money(float(amount))} | kind={expense_kind_value} | "
    ...
    st.markdown("---")
    st.markdown("**Add Monthly Adjustment (Owner Finance Adjustment)**")
    st.caption(
    ...
    active_invoice_options = (
        invoice_option_labels_transaction
        if kind_default_label == "Transaction (invoice/day level)"
        else ["Not linked to invoice"]
    )
    if not active_invoice_options:
        active_invoice_options = ["Not linked to invoice"]
    invoice_default_index = (
        active_invoice_options.index(selected_invoice_label)
        if selected_invoice_label in active_invoice_options
        else 0
    )
    ...
    with st.form(f"expense_edit_form_{selected_expense_id}", clear_on_submit=False):
        ...
        edited_link_label = f3.selectbox(
            "Link to Invoice"
            if edited_kind_label != "Transaction (invoice/day level)"
            else "Link to Confirmed Invoice *",
            options=(
                invoice_option_labels_transaction
                if edited_kind_label == "Transaction (invoice/day level)"
                else ["Not linked to invoice"]
            )
            or ["Not linked to invoice"],
    ...
    if save_edit:
        if edited_amount <= 0:
            ...
            try:
                ...
                if (
                    str(edited_category).strip().lower() == "re-rental"
                ...
                finance_audit_log(
                    ...
                    action_type="update",
                    notes=(
                        f"{edited_category} | {money(float(edited_amount))} | kind={edited_kind_value} | "
...
def render_supplier_rerental() -> None:
    ...
    with st.form("supplier_rerental_form", clear_on_submit=True):
        ...
        b1, b2 = st.columns(2)
        link_label = b1.selectbox("Link to Invoice *", options)
...
def render_reports(report_start_month: str) -> None:
    ...
    st.markdown("**Daily Summary (Event-Date Linked)**")
    if daily.empty:
        st.caption("No daily finance rows yet.")
    else:
        daily_show = daily.copy().sort_values("day")
        for col in [
            "revenue",
            "cash_collected",
    ...
    with export_col1:
        st.markdown("Monthly Finance Pack (PDF + CSV)")
        month_choices = monthly["month"].dropna().astype(str).drop_duplicates().tolist()
        month_choices = sorted(month_choices, reverse=True)
        if not month_choices:
            st.caption("No months available.")
        else:
            selected_pack_month = st.selectbox(
                "Select Month",
                options=month_choices,
                key="reports_export_month_selector",
            )
            month_row = monthly[monthly["month"] == selected_pack_month].iloc[-1]
            month_label = str(month_row.get("month_label", selected_pack_month))
            month_invoice_rows = invoices[invoices["month"] == selected_pack_month].copy()
            if not month_invoice_rows.empty:
                month_invoice_rows["event_date"] = pd.to_datetime(
                    month_invoice_rows["event_date"], errors="coerce"
                ).dt.date.astype("string")

            month_kpis = [
                ("Revenue", money(float(month_row["revenue"]))),
                ("Expenses", money(float(month_row["total_expenses"]))),
                ("Net Profit", money(float(month_row["net_profit_after_adjustments"]))),
                ("Outstanding", money(float(month_row.get("outstanding_receivables", 0.0)))),
            ]
            month_table = month_invoice_rows[
                [
                    "invoice_number",
                    "event_date",
                    "customer_name",
            ...
            ].copy() if not month_invoice_rows.empty else pd.DataFrame(
                columns=[
                    "invoice_number",
                    "event_date",
                    "customer_name",
            ...
            for money_col in ["revenue", "amount_paid", "amount_outstanding"]:
                if money_col in month_table.columns:
                    month_table[money_col] = pd.to_numeric(
                        month_table[money_col], errors="coerce"
                    ).fillna(0.0).map(money)

            month_pdf = build_finance_summary_pdf(
                title=f"Headline Rentals - Monthly Finance Pack",
                subtitle=f"{month_label} ({selected_pack_month})",
                kpis=month_kpis,
                table_title="Confirmed Orders Snapshot",
                table_df=month_table,
            )
            month_zip = io.BytesIO()
            with zipfile.ZipFile(month_zip, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                zf.writestr(
                    f"monthly_finance_{selected_pack_month}.csv",
                    monthly[monthly["month"] == selected_pack_month].to_csv(index=False),
                )
                zf.writestr(
                    f"confirmed_orders_{selected_pack_month}.csv",
                    month_invoice_rows.to_csv(index=False),
                )
                zf.writestr(f"monthly_finance_{selected_pack_month}.pdf", month_pdf)
            month_zip.seek(0)
            st.download_button(
                "Download Monthly Pack",
                data=month_zip.getvalue(),
                file_name=f"headline_monthly_finance_pack_{selected_pack_month}.zip",
                mime="application/zip",
                key="reports_download_monthly_pack_btn",
            )

    with export_col2:
        st.markdown("Year-End Summary (PDF + CSV)")
        year_choices = sorted([int(y) for y in yearly["year"].dropna().unique()], reverse=True)
        if not year_choices:
            st.caption("No years available.")
        else:
            selected_year = int(
                st.selectbox(
                    "Select Year",
                    options=year_choices,
                    key="reports_export_year_selector",
                )
            )
            year_row = yearly[yearly["year"] == selected_year].iloc[-1]
            year_months = monthly[monthly["year"] == selected_year].copy()
            year_kpis = [
                ("Revenue", money(float(year_row["revenue"]))),
                ("Expenses", money(float(year_row["total_expenses"]))),
                ("Net Profit", money(float(year_row["net_profit_after_adjustments"]))),
                ("Cash Collected", money(float(year_row.get("cash_collected", 0.0)))),
            ]
            year_table = year_months[
                [
                    "month_label",
                    "revenue",
                    "cash_collected",
            ...
            ].copy() if not year_months.empty else pd.DataFrame(
                columns=[
                    "month_label",
                    "revenue",
                    "cash_collected",
            ...
            for money_col in ["revenue", "cash_collected", "total_expenses", "net_profit_after_adjustments"]:
                if money_col in year_table.columns:
                    year_table[money_col] = pd.to_numeric(
                        year_table[money_col], errors="coerce"
                    ).fillna(0.0).map(money)

            year_pdf = build_finance_summary_pdf(
                title="Headline Rentals - Year-End Summary",
                subtitle=f"Year {selected_year}",
                kpis=year_kpis,
                table_title="Monthly Breakdown",
                table_df=year_table,
            )
            year_zip = io.BytesIO()
            with zipfile.ZipFile(year_zip, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                zf.writestr(
                    f"year_end_summary_{selected_year}.csv",
                    year_months.to_csv(index=False),
                )
                zf.writestr(
                    f"year_end_summary_{selected_year}.pdf",
                    year_pdf,
                )
            year_zip.seek(0)
            st.download_button(
                "Download Year-End Pack",
                data=year_zip.getvalue(),
                file_name=f"headline_year_end_summary_{selected_year}.zip",
                mime="application/zip",
                key="reports_download_year_pack_btn",
            )

    st.markdown("**Tax Pack (GCT-Ready)**")
    st.caption(
    ...
    st.markdown("**Bank Reconciliation Import**")
    st.caption(
...
def render_importer() -> None:
    render_section_shell(
        ...
        "Load historical sheets and Shopify exports into the app database.",
    )
    st.caption(
...
def render_deposit_due_tracker(report_start_month: str) -> None:
    ...
    st.caption(
        "Track deposit-balance invoices only. Profit and full Finance Hub details remain locked."
    )
    st.caption("This tracker shows all outstanding deposit balances (not filtered by report start month).")
    ...
    tracker = invoice_level[
        (invoice_level["amount_outstanding"] > 0.01)
        & (
            invoice_level["payment_status"]
            .fillna("")
            .astype(str)
            .str.strip()
            .str.lower()
        ...
        & (
            pd.to_numeric(
                invoice_level.get("deposit_balance_enabled", 0),
                errors="coerce",
            )
            .fillna(0)
            .astype(int)
    ...
    if tracker.empty:
        st.success(
            "No 50% deposit confirmed-order balances outstanding right now."
        )
        return
    ...
    if tracker.empty:
        st.info("Deposit-balance invoices do not have valid event dates for due tracking.")
        return
    ...
    st.caption(
        "This section tracks only confirmed orders saved with the `50% Deposit (Balance Later)` option."
    )
    with st.expander("Show Related Invoice Details", expanded=False):
        try:
            ...
            st.caption(f"Could not load invoice details: {exc}")
    ...
    with st.form("deposit_tracker_update_form", clear_on_submit=True):
        ...
        finance_password = st.text_input(
            ...
            help="Required before posting to Finance Hub calculations.",
        )
...
def build_global_search_results(query: str, finance_unlocked: bool) -> pd.DataFrame:
    ...
    invoice_rows = invoice_options(include_quotes=True, confirmed_only=False)
    ...
    inventory_rows = load_inventory_snapshot()
...
def render_quick_command_bar(
    available_sections: list[str],
    report_start_month: str,
    alert_window_days: int,
) -> None:
    st.markdown("**Quick Command (WattBot Routing)**")
    st.caption("Type a command. Example: `open build invoice`, `inventory status`, `ops brief`.")

    with st.form("inline_quick_command_form", clear_on_submit=True):
        command_text = st.text_input(
            "Command",
            value="",
            placeholder="e.g. open today home",
        )
        run_command = st.form_submit_button("Run Command")
    ...
    if run_command:
        ...
        wattbot_append("assistant", response)
        st.session_state["inline_command_last_response"] = response
        if needs_rerun:
            st.rerun()

    last_response = str(st.session_state.get("inline_command_last_response", "") or "").strip()
    if last_response:
