...
def render_dashboard(report_start_month: str, alert_window_days: int) -> None:
    ...
    def _collect_month_tokens(*frames: pd.DataFrame) -> list[str]:
        ...
        for frame in frames:
            ...
            for token in series:
                if re.fullmatch(r"\d{4}-\d{2}", token or ""):
                    out.add(token)
    ...
    month_label_map = {
        token: pd.Period(token, freq="M").strftime("%b %Y")
    ...
    dashboard_scope_mode = scope_col1.selectbox(
        ...
        options=["Current Month", "Specific Month", "Specific Year", "All Data"],
        index=0,
    ...
    if dashboard_scope_mode == "Specific Month":
        default_month_index = (
            available_month_tokens.index(current_month_token)
            if current_month_token in available_month_tokens
            else 0
        )
        selected_scope_month = scope_col2.selectbox(
            "Select Month",
            options=available_month_tokens,
            index=default_month_index,
            format_func=lambda token: month_label_map.get(token, token),
            key="dashboard_scope_month_selector",
        )
        ...
        scope_col2.caption("Use the selector on the left to switch by month or year.")

    if dashboard_scope_mode == "Current Month":
        dashboard_scope_label = f"Current Month ({month_label_map.get(current_month_token, current_month_token)})"
    ...
    def _filter_dashboard_window(frame: pd.DataFrame) -> pd.DataFrame:
        if frame.empty or "month" not in frame.columns:
            return frame.copy()
        out = frame.copy()
        month_series = out["month"].fillna("").astype(str).str.strip()
        if dashboard_scope_mode == "Current Month":
            return out[month_series == current_month_token].copy()
        if dashboard_scope_mode == "Specific Month":
            return out[month_series == selected_scope_month].copy()
        if dashboard_scope_mode == "Specific Year":
            year_series = pd.to_numeric(month_series.str.slice(0, 4), errors="coerce")
            return out[year_series == float(selected_scope_year)].copy()
        return out

    monthly = _filter_dashboard_window(monthly_all)
    invoice_level = _filter_dashboard_window(invoice_level_all)
    expenses_view = _filter_dashboard_window(expenses_all)
    ...
    if not upcoming.empty:
        ...
        if dashboard_scope_mode == "Current Month":
            upcoming = upcoming[upcoming_event_dates.dt.strftime("%Y-%m") == current_month_token].copy()
        elif dashboard_scope_mode == "Specific Month":
            upcoming = upcoming[upcoming_event_dates.dt.strftime("%Y-%m") == selected_scope_month].copy()
        elif dashboard_scope_mode == "Specific Year":
            upcoming = upcoming[upcoming_event_dates.dt.year == int(selected_scope_year)].copy()
...
def render_invoices() -> None:
    ...
    item_name_column = st.column_config.TextColumn(
        "Item Name",
        help=(
            "Type any item name (e.g. Throne Chair), then press Enter or click out of the cell "
            "to commit. Inventory suggestions are shown below."
        ),
    )

    st.caption(
        "Edit the table directly. Press Enter (or click outside the cell) to commit values, "
        "then tap `Apply Table Changes` to lock your latest rows before bundle actions."
    )
