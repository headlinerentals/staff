...
def render_top_nav_bar(active_section: str) -> None:
    ...
    st.markdown(
        ...
        <script>
            (() => {{
                if (window.__hrNavBindInit) return;
                window.__hrNavBindInit = true;
                const bindLinks = () => {{
                    const nodes = document.querySelectorAll(".hr-nav-link[data-sec]");
                    nodes.forEach((node) => {{
                        if (node.dataset.boundNav === "1") return;
                        node.dataset.boundNav = "1";
                        node.addEventListener("click", (event) => {{
                            event.preventDefault();
                            const sec = (node.dataset.sec || "").trim();
                            if (!sec) return;
...
def render_invoices() -> None:
    ...
    if item_name_suggestions:
        item_name_column = st.column_config.SelectboxColumn(
            "Item Name",
            options=item_name_suggestions,
            help="Type to search inventory names and avoid duplicate variations.",
        )
    else:
        item_name_column = st.column_config.TextColumn("Item Name")

    st.caption(
        "Edit the table directly. Then tap `Apply Table Changes` to lock your latest rows before bundle actions."
    )
    ...
    apply_note_col.caption(
        "Use this before saving a bundle or switching to another section."
    )
