...
def render_invoices() -> None:
    ...
    st.caption(
        "Performance mode: edit the table, then tap `Apply Table Changes` once. "
        "This prevents lag and double-entry issues on mobile."
    )
    with st.form("invoice_items_fast_editor_form", clear_on_submit=False):
        ...
        apply_item_changes = st.form_submit_button(
            "Apply Table Changes",
            use_container_width=True,
        )

    if apply_item_changes:
        items = normalize_invoice_items_df(items_edited)
        st.session_state["invoice_items_editor_data"] = items[
            ["item_name", "item_type", "quantity", "unit_price"]
        ].copy()
        st.success("Item table updated.")
    else:
        items = normalize_invoice_items_df(
            st.session_state.get("invoice_items_editor_data", items_editor_source)
        )
    ...
    st.caption("Before saving, tap `Apply Table Changes` so the latest item edits are included.")
    ...
    with st.expander("Attach Files (PNG/PDF) to Invoices", expanded=False):
        inv = invoice_options()
        if inv.empty:
    ...
    with st.expander("Build Log (Quotes + Invoices)", expanded=False):
