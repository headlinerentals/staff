...
def google_maps_distance(origin: str, destination: str, api_key: str) -> tuple[float, float, str]:
    """
    Uses Google Maps Platform Geocoding API + Distance Matrix API.
    Requires a Google Cloud project with billing enabled.
    """
    if not api_key:
        return 0.0, 0.0, "Google Maps API key is missing. Add GOOGLE_MAPS_API_KEY in Streamlit secrets."
    if not origin.strip() or not destination.strip():
        return 0.0, 0.0, "Origin and destination are required for automatic delivery calculation."
    ...
    def _distance_lookup(origins: str, destinations: str) -> tuple[float, float, str]:
        ...
        status = str(payload.get("status", ""))
        if status != "OK":
            return 0.0, 0.0, f"Distance lookup failed. {_google_status_help(status, str(payload.get('error_message', '') or ''))}"
        ...
        if element_status != "OK":
            return 0.0, 0.0, f"No driving route found. {_google_status_help(element_status)}"
        distance_km = float(element["distance"]["value"]) / 1000.0
        duration_min = float(element["duration"]["value"]) / 60.0
        return distance_km, duration_min, ""

    try:
        origin_lat, origin_lng, origin_error = _geocode(origin)
        if origin_error:
            ...
            for origin_candidate in _address_variants(origin):
                for dest_candidate in _address_variants(destination):
                    distance_km, duration_min, direct_error = _distance_lookup(origin_candidate, dest_candidate)
                    if not direct_error:
                        return distance_km, duration_min, ""
            return 0.0, 0.0, origin_error
        dest_lat, dest_lng, dest_error = _geocode(destination)
        if dest_error:
            for dest_candidate in _address_variants(destination):
                distance_km, duration_min, direct_error = _distance_lookup(
                    f"{origin_lat},{origin_lng}",
                    dest_candidate,
                )
                if not direct_error:
                    return distance_km, duration_min, ""
            return 0.0, 0.0, dest_error

        return _distance_lookup(f"{origin_lat},{origin_lng}", f"{dest_lat},{dest_lng}")
    except Exception as exc:
        return 0.0, 0.0, f"Google Maps lookup failed: {exc}"
...
def render_invoices() -> None:
    ...
    state_defaults = {
        ...
        "invoice_auto_setup_hourly_rate_input": float(
            delivery_setting_float("setup_hourly_rate", 1200.0)
        ),
    ...
    with st.expander("Automatic fee rate settings", expanded=False):
        ...
        wear_per_vehicle_round_trip = rs5.number_input(
            "Vehicle Wear Fee Per Vehicle Per Round-Trip Pair (JMD)",
            min_value=0.0,
            step=500.0,
            value=delivery_setting_float("wear_per_vehicle_round_trip", 2500.0),
            key="rate_wear_per_vehicle_round_trip_input",
        )
        ...
        loading_buffer_minutes = rs5b.number_input(
            ...
            help="Extra handling time added to each round trip in hourly delivery mode.",
        )
        rs6, rs7, rs8 = st.columns(3)
        ...
        setup_hourly_rate = rs8.number_input(
            "Set-Up Labour Hourly Rate Per Person (JMD)",
            min_value=0.0,
            step=250.0,
            value=delivery_setting_float("setup_hourly_rate", 1200.0),
            key="rate_setup_hourly_rate_input",
        )
        ...
        with vp1:
            v1_name = st.text_input(
                "Vehicle 1 Name",
                value=get_delivery_setting("vehicle_1_name", "Truck/Van"),
        ...
        with vp2:
            v2_name = st.text_input(
                "Vehicle 2 Name",
                value=get_delivery_setting("vehicle_2_name", "Second vehicle"),
        ...
        if st.button("Save Rate Settings", key="save_fee_rate_settings_btn", type="secondary"):
            for key, value in {
                ...
                "wear_per_vehicle_round_trip": wear_per_vehicle_round_trip,
            ...
            st.session_state["invoice_auto_setup_hourly_rate_input"] = float(setup_hourly_rate)
        ...
        with st.expander("Automatic Delivery Calculator", expanded=True):
            ...
            if "invoice_auto_delivery_trip_days_input" not in st.session_state:
                st.session_state["invoice_auto_delivery_trip_days_input"] = int(
                    max(1, st.session_state["invoice_auto_delivery_round_trips_input"])
                )
            ...
            round_trip_count = dcalc1.selectbox(
                ...
                help="1 = same-day delivery + pickup. 2 = separate delivery and collection trips.",
            )
            ...
            trip_days = dcalc3.number_input(
                ...
                help="Defaults to round trips, but you can override it for multiple same-day loads.",
            )
            ...
            if st.button("Calculate Delivery Route", key="invoice_auto_delivery_route_btn", type="secondary"):
                one_way_km, one_way_min, route_error = google_maps_distance(
                    origin_address,
                    destination_address,
                ...
                if not bool(st.session_state.get("invoice_auto_delivery_billing_mode_manually_set", False)):
                    st.session_state["invoice_auto_delivery_billing_mode_input"] = (
                        "Hourly Rate" if float(one_way_min or 0.0) < 90.0 else "Day Rate"
                    )
            ...
            if "invoice_auto_delivery_billing_mode_input" not in st.session_state:
                st.session_state["invoice_auto_delivery_billing_mode_input"] = (
                    "Hourly Rate" if one_way_min < 90.0 else "Day Rate"
                )
            ...
            vehicle_rows = []
            if use_v1:
                vehicle_rows.append((v1_name, v1_fuel_type, float(v1_efficiency)))
            if use_v2:
                vehicle_rows.append((v2_name, v2_fuel_type, float(v2_efficiency)))
            fuel_cost = 0.0
            for _, _, efficiency in vehicle_rows:
                fuel_cost += (total_delivery_km / max(float(efficiency), 0.1)) * float(fuel_price_per_litre)
            one_way_drive_time_hours = float(one_way_min) / 60.0
            loading_buffer_hours = float(loading_buffer_minutes) / 60.0
            hourly_labour_hours_per_round_trip = (2.0 * one_way_drive_time_hours) + loading_buffer_hours
            hourly_labour_hours = float(round_trip_count) * hourly_labour_hours_per_round_trip
            if billing_mode == "Hourly Rate":
                labour_cost = (
                    int(delivery_crew_size)
                    * float(round_trip_count)
                    * hourly_labour_hours_per_round_trip
                    * float(delivery_hourly_rate_per_person)
                )
                labour_label = (
                    f"Hourly ({int(delivery_crew_size)} crew x {int(round_trip_count)} round trip(s) "
                    f"x {hourly_labour_hours_per_round_trip:,.2f}h x {money(delivery_hourly_rate_per_person)}/hr)"
                )
            else:
                labour_cost = float(delivery_labour_day_rate) * int(delivery_crew_size) * int(trip_days)
                labour_label = (
                    f"Day Rate ({int(delivery_crew_size)} crew x {int(trip_days)} day(s) "
                    f"x {money(delivery_labour_day_rate)})"
                )
            wear_cost = float(wear_per_vehicle_round_trip) * len(vehicle_rows) * int(round_trip_count)
            ...
            delivery_subtotal_before_margin = fuel_cost + labour_cost + wear_cost + risk_cost
            ...
            internal_breakdown = pd.DataFrame(
                [
                    ...
                    {"Component": "One-way Drive Time", "Value": f"{one_way_min:,.0f} min"},
                    {"Component": "Total Delivery Distance", "Value": f"{total_delivery_km:,.2f} km"},
                    {"Component": "Fuel", "Value": money(fuel_cost)},
                    ...
                    {"Component": "Vehicle Wear", "Value": money(wear_cost)},
                    {
    ...
    if setup_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Set-Up Calculator", expanded=True):
            ...
            setup_rate_override = sc3.number_input(
                "Hourly Labour Rate Per Person (JMD)",
                min_value=0.0,
                step=250.0,
                value=float(setup_hourly_rate),
