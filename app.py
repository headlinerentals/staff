...
def render_invoices() -> None:
    ...
    with st.expander("Automatic fee rate settings", expanded=False):
        ...
        if st.button("Save Rate Settings", key="save_fee_rate_settings_btn", type="secondary"):
            ...
            try:
                backup_result = create_db_backup_snapshot(reason="rate_settings_save", force=True)
                if backup_result:
                    st.caption("Backup saved and pushed to cloud storage.")
                else:
                    st.warning(
                        "Rate settings saved locally, but the cloud backup could not be "
                        "confirmed. Consider using Download Rate Settings Backup below as "
                        "an extra safety copy."
                    )
            except Exception:
                st.warning(
                    "Rate settings saved locally, but the cloud backup step raised an "
                    "error. Consider using Download Rate Settings Backup below as an "
                    "extra safety copy."
                )
