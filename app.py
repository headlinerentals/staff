...
def load_public_holidays_table() -> pd.DataFrame:
    ...
    for col in ["holiday_date", "holiday_name"]:
        if col not in table.columns:
            table[col] = ""
    table["holiday_date"] = table["holiday_date"].astype(str).str.strip()
...
def render_invoices() -> None:
    ...
    if delivery_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Delivery Calculator", expanded=True):
            ...
            vehicle_rows = []
            if use_v1:
                vehicle_rows.append((v1_name, v1_fuel_type, float(v1_efficiency), float(v1_congestion_sensitivity)))
            if use_v2:
                vehicle_rows.append((v2_name, v2_fuel_type, float(v2_efficiency), float(v2_congestion_sensitivity)))
            ...
            for vehicle_name, _fuel_type, efficiency, congestion_sensitivity in vehicle_rows:
