...
def google_maps_distance(origin: str, destination: str, api_key: str) -> tuple[float, float, str]:
    ...
    def _geocode(address: str) -> tuple[float, float, str]:
        ...
        if status != "OK" or not payload.get("results"):
            return 0.0, 0.0, f"Could not geocode '{address}'. Google status: {status or 'UNKNOWN'}."
    ...
    try:
        ...
        status = str(payload.get("status", ""))
        if status != "OK":
            return 0.0, 0.0, f"Distance lookup failed. Google status: {status or 'UNKNOWN'}."
        element = payload["rows"][0]["elements"][0]
        element_status = str(element.get("status", ""))
        if element_status != "OK":
            return 0.0, 0.0, f"No driving route found. Google status: {element_status or 'UNKNOWN'}."
...
def render_invoices() -> None:
    ...
    with st.expander("Automatic fee rate settings", expanded=False):
        ...
        origin_address = rs1.text_input(
            ...
            value=get_delivery_setting("origin_address", "61 West Main Drive, Marverly, Kingston 20"),
