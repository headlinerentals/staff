...
def _backup_snapshot_summary_fallback(snapshot_path: str) -> dict[str, object]:
    ...
    with sqlite3.connect(target) as conn:
        summary["operational_rows"] = int(
            sum(int(_scalar(conn, f"SELECT COUNT(*) FROM {table}") or 0) for table in operational_tables)
        )
        ...
        summary["inventory_movements"] = int(_scalar(conn, "SELECT COUNT(*) FROM inventory_movements") or 0)
        try:
...
export_delivery_settings_json = _db_attr("export_delivery_settings_json")
import_delivery_settings_json = _db_attr("import_delivery_settings_json")
get_last_remote_push_info = _db_attr("get_last_remote_push_info")
...
backup_snapshot_summary = _db_attr("backup_snapshot_summary", _backup_snapshot_summary_fallback)
...
def setup_minutes_defaults() -> list[dict[str, object]]:
    return [
        ...
        {"category": "linens/accessories", "keywords": "chair sash,chair sashes,chair cover,chair covers,table cloth,table cloths,table cover,table covers,table runner,table runners", "minutes_per_unit": 0.50},
        {"category": "small decor", "keywords": "table number,table numbers", "minutes_per_unit": 0.15},
        {"category": "igloo/cooler", "keywords": "igloo,cooler,coolers", "minutes_per_unit": 3.0},
        {"category": "food warmer", "keywords": "warmer,chafing,food warmer,chaffing", "minutes_per_unit": 3.0},
        {"category": "lighting/decor", "keywords": "flood light,led flower set,stage light,string light,string lights,led stools", "minutes_per_unit": 1.50},
        {"category": "bar/lounge", "keywords": "bar counter,bar stool,bar stools,ottoman,ottermon", "minutes_per_unit": 2.50},
        {"category": "catering equipment", "keywords": "coal stove,jerk pan,popcorn machine,cotton candy machine,snowcone machine", "minutes_per_unit": 4.0},
        {"category": "turf/grass", "keywords": "40x20 grass,grass wall", "minutes_per_unit": 15.0},
        {"category": "Tent", "keywords": "10x10 tent,10x20 tent,20x20 tent,backdrop", "minutes_per_unit": 20.0},
        {"category": "Bounce", "keywords": "bounce about,bounce castle", "minutes_per_unit": 25.0},
        {"category": "Trampoline", "keywords": "trampoline", "minutes_per_unit": 35.0},
        {"category": "tent sidewalls", "keywords": "sidewall,sidewalls", "minutes_per_unit": 5.0},
        {"category": "photo booth", "keywords": "360 photo booth,photo booth", "minutes_per_unit": 40.0},
        {"category": "small props", "keywords": "half drum", "minutes_per_unit": 3.0},
        {"category": "excluded/non-item", "keywords": "delivery,delivery #2,set-up,none", "minutes_per_unit": 0.0},
...
def public_holiday_defaults() -> list[dict[str, object]]:
    return [
        {"holiday_date": "2027-01-01", "holiday_name": "New Year's Day"},
        {"holiday_date": "2027-02-10", "holiday_name": "Ash Wednesday"},
        {"holiday_date": "2027-03-26", "holiday_name": "Good Friday"},
        {"holiday_date": "2027-03-29", "holiday_name": "Easter Monday"},
        {"holiday_date": "2027-05-23", "holiday_name": "National Labour Day"},
        {"holiday_date": "2027-05-24", "holiday_name": "National Labour Day (observed)"},
        {"holiday_date": "2026-08-01", "holiday_name": "Emancipation Day"},
        {"holiday_date": "2026-08-06", "holiday_name": "Independence Day"},
        {"holiday_date": "2026-10-19", "holiday_name": "National Heroes Day"},
        {"holiday_date": "2026-12-25", "holiday_name": "Christmas Day"},
        {"holiday_date": "2026-12-26", "holiday_name": "Boxing Day"},
    ]
...
def effective_setup_hourly_rate(
    ...
    cost_rate = (
        ...
        else delivery_setting_float("setup_labour_cost_rate", 1000.0)
    )
    margin_pct = (
...
def render_invoices() -> None:
    ...
    with st.expander("Automatic fee rate settings", expanded=False):
        ...
        origin_address = rs1.text_input(
            ...
            value=get_delivery_setting("origin_address", "61 West Main Drive, Marverley, Kingston 20, Jamaica"),
        ...
        fuel_price_per_litre = rs2.number_input(
            ...
            min_value=0.0,
            step=10.0,
            value=delivery_setting_float("fuel_price_per_litre", 225.0),
        ...
        delivery_hourly_rate_per_person = rs5a.number_input(
            ...
            min_value=0.0,
            step=100.0,
            value=delivery_setting_float("delivery_hourly_rate_per_person", 1000.0),
        ...
        rs5c, rs5d = st.columns(2)
        staff_pay_floor_hourly = rs5c.number_input(
            "Staff Pay Floor - Hourly (Combined, JMD/hr)",
            min_value=0.0,
            step=25.0,
            value=delivery_setting_float("staff_pay_floor_hourly", 875.00),
            key="rate_staff_pay_floor_hourly_input",
            help=(
                "Safety net, not a price change: Delivery/Collection labour for an Hourly-mode leg never bills "
                "below this combined per-hour rate, regardless of what the normal formula computes. Default "
                "reflects the full real combined cost: Driver pay (J$500/hr-equivalent) plus the Assistant's "
                "(J$375/hr-equivalent) in full, since both are fully engaged during the delivery/setup task itself."
            ),
        )
        staff_pay_floor_daily = rs5d.number_input(
            "Staff Pay Floor - Day Rate (Combined, JMD/day)",
            min_value=0.0,
            step=250.0,
            value=delivery_setting_float("staff_pay_floor_daily", 7000.0),
            key="rate_staff_pay_floor_daily_input",
            help=(
                "Safety net, not a price change: Delivery/Collection labour for a Day-Rate-mode leg never bills "
                "below this combined per-day rate. Default reflects the full real combined cost: Driver pay "
                "(J$4,000/day) plus the Assistant's (J$3,000/day) in full, since both are fully engaged during "
                "the delivery/setup task itself."
            ),
        )
        ...
        risk_buffer_percent = rs6.number_input(
            ...
            min_value=0.0,
            step=1.0,
            value=delivery_setting_float("risk_buffer_percent", 3.0),
        ...
        setup_labour_cost_rate = rs8.number_input(
            ...
            min_value=0.0,
            step=25.0,
            value=delivery_setting_float("setup_labour_cost_rate", 1000.0),
        ...
        public_holiday_premium_pct = prem1.number_input(
            ...
            min_value=0.0,
            step=5.0,
            value=delivery_setting_float("public_holiday_premium_pct", 75.0),
        ...
        with vp1:
            ...
            v1_efficiency = st.number_input(
                ...
                min_value=1.0,
                step=0.5,
                value=delivery_setting_float("vehicle_1_efficiency", 10.5),
            ...
            v1_congestion_sensitivity = st.number_input(
                ...
                max_value=1.0,
                step=0.05,
                value=delivery_setting_float("vehicle_1_congestion_sensitivity", 0.30),
        ...
        with vp2:
            ...
            v2_congestion_sensitivity = st.number_input(
                ...
                max_value=1.0,
                step=0.05,
                value=delivery_setting_float("vehicle_2_congestion_sensitivity", 0.50),
        ...
        if st.button("Save Rate Settings", key="save_fee_rate_settings_btn", type="secondary"):
            for key, value in {
                ...
                "staff_pay_floor_hourly": staff_pay_floor_hourly,
                "staff_pay_floor_daily": staff_pay_floor_daily,
            ...
            # Rate Settings previously only wrote to the local DB — nothing ever
            # triggered a remote push, so a save here could sit unprotected until
            # some unrelated invoice action happened to trigger one. Streamlit has
            # no dependable "about to sleep/redeploy" event, so the reliable
            # equivalent is backing up immediately after every save, not waiting
            # for one before shutdown.
            # Rate Settings previously only wrote to the local DB — nothing ever
            # triggered a remote push, so a save here could sit unprotected until
            # some unrelated invoice action happened to trigger one. Streamlit has
            # no dependable "about to sleep/redeploy" event, so the reliable
            ...
            # for one before shutdown.
            try:
                create_db_backup_snapshot(reason="rate_settings_save", force=True)
                push_info = get_last_remote_push_info() or {}
                if bool(push_info.get("ok")):
                    st.caption(
                        f"Backup saved and confirmed pushed to cloud storage "
                        f"at {push_info.get('at', '')}."
                    )
                else:
                    st.warning(
                        "Rate settings saved locally, but the cloud push did NOT "
                        "succeed. Real reason from the backup system: "
                        f"\u201c{push_info.get('reason', 'No diagnostic info was recorded.')}\u201d "
                        "Use Download Rate Settings Backup below as an immediate "
                        "safety copy until this is resolved."
                    )
            except Exception as exc:
                st.warning(
                    f"Rate settings saved locally, but the cloud backup step raised "
                    f"an error before it could even attempt the push: {exc}. Use "
                    "Download Rate Settings Backup below as an immediate safety copy."
                )
            st.success("Rate settings saved.")

        st.markdown("---")
        st.caption(
            "Rate Settings Backup — an independent safety copy separate from the "
            "automatic cloud sync. Download one after making changes you don't want "
            "to risk re-typing; import one to restore everything in a single step."
        )
        rsb1, rsb2 = st.columns(2)
        with rsb1:
            try:
                settings_backup_json = export_delivery_settings_json()
            except Exception:
                settings_backup_json = ""
            st.download_button(
                "Download Rate Settings Backup",
                data=settings_backup_json,
                file_name=f"rate_settings_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
                key="rate_settings_backup_download_btn",
                disabled=not bool(settings_backup_json),
            )
        with rsb2:
            uploaded_settings_backup = st.file_uploader(
                "Import Rate Settings from Backup",
                type=["json"],
                key="rate_settings_backup_uploader",
            )
            if uploaded_settings_backup is not None and st.button(
                "Restore This Backup", key="rate_settings_backup_restore_btn"
            ):
                try:
                    restored_count = import_delivery_settings_json(
                        uploaded_settings_backup.read().decode("utf-8")
                    )
                    try:
                        create_db_backup_snapshot(reason="rate_settings_import", force=True)
                    except Exception:
                        pass
                    st.success(
                        f"Restored {restored_count} Rate Settings values. "
                        "Reopen this section to see the restored values."
                    )
                    st.rerun()
                except Exception as exc:
    ...
    if delivery_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Delivery Calculator", expanded=True):
            ...
            if billing_mode == "Hourly Rate":
                ...
                # Staff Pay Floor: never bill either leg below the real combined staff
                # cost for the hours involved, regardless of what the normal formula
                # computes. staff_pay_floor_hourly already represents BOTH crew members
                # combined, so it multiplies leg hours directly with no crew_size factor.
                delivery_labour_floor = float(staff_pay_floor_hourly) * delivery_labour_hours
                collection_labour_floor = float(staff_pay_floor_hourly) * collection_labour_hours
                delivery_floor_applied = delivery_labour_floor > delivery_labour_cost
                collection_floor_applied = collection_labour_floor > collection_labour_cost
                delivery_labour_cost = max(delivery_labour_cost, delivery_labour_floor)
                collection_labour_cost = max(collection_labour_cost, collection_labour_floor)
                ...
                labour_label = (
                    ...
                    + (" [Staff Pay Floor applied]" if (delivery_floor_applied or collection_floor_applied) else "")
                )
                labour_detail_rows = [
                    {
                        ...
                        "Value": (
                            ...
                            + (" - Staff Pay Floor applied" if delivery_floor_applied else "")
                        ),
                    },
                    {
                        ...
                        "Value": (
                            ...
                            + (" - Staff Pay Floor applied" if collection_floor_applied else "")
                        ),
                    },
                ]
            else:
                ...
                if int(trip_days) >= 2:
                    ...
                    # Staff Pay Floor: same safety net as Hourly mode, scaled per day
                    # instead of per hour. staff_pay_floor_daily already represents
                    # both crew members combined.
                    delivery_labour_floor = float(staff_pay_floor_daily)
                    collection_labour_floor = float(staff_pay_floor_daily) * collection_day_count
                    delivery_floor_applied = delivery_labour_floor > delivery_labour_cost
                    collection_floor_applied = collection_labour_floor > collection_labour_cost
                    delivery_labour_cost = max(delivery_labour_cost, delivery_labour_floor)
                    collection_labour_cost = max(collection_labour_cost, collection_labour_floor)
                    ...
                    same_day_floor_applied = float(staff_pay_floor_daily) > labour_cost
                    labour_cost = max(labour_cost, float(staff_pay_floor_daily))
                    labour_label = (
                        f"Day Rate ({int(delivery_crew_size)} crew x 1 day x "
                        f"{money(delivery_labour_day_rate)}, highest same-day premium applied once)"
                        + (" [Staff Pay Floor applied]" if same_day_floor_applied else "")
                    )
                    labour_detail_rows = [
                        {
    ...
    if setup_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Set-Up Calculator", expanded=True):
            ...
            raw_calculated_setup_amount = round(setup_hours * float(setup_rate_override), 2)
            calculated_setup_amount = (
                float(math.ceil(raw_calculated_setup_amount / 500.0) * 500.0)
                if raw_calculated_setup_amount > 0
                else 0.0
            )
            if setup_rows:
            ...
            st.caption(
                ...
                f"Raw amount: {money(raw_calculated_setup_amount)} -> rounded up to "
                f"Suggested Set-Up and Breakdown Fee: {money(calculated_setup_amount)}"
            )
            st.caption(
    ...
    if st.button("Create Backup Now", key="mobile_team_force_backup_btn"):
        try:
            ...
            if created is None:
                ...
                st.success(f"Local backup created: {created}")
                push_info = get_last_remote_push_info() or {}
                if bool(push_info.get("ok")):
                    st.success(
                        f"Cloud push confirmed at {push_info.get('at', '')}."
                    )
                else:
                    st.warning(
                        "Cloud push did NOT succeed. Real reason: "
                        f"\u201c{push_info.get('reason', 'No diagnostic info was recorded.')}\u201d"
                    )
        except Exception as exc:
        ...
        if backup_catalog.empty:
            ...
            if st.button(
                ...
                try:
                    ...
                    if not isinstance(restore_result.get("live_summary"), dict):
                        try:
                            restore_result["live_summary"] = backup_snapshot_summary(str(DB_PATH))
                        except Exception:
                            restore_result["live_summary"] = {}
