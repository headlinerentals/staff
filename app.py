...
def setup_minutes_defaults() -> list[dict[str, object]]:
    return [
        ...
        {"category": "place setting", "keywords": "plate,plates,glass,glasses,cutlery,napkin", "minutes_per_unit": 0.75},
...
def render_invoices() -> None:
    ...
    with st.expander("Automatic fee rate settings", expanded=False):
        ...
        wear_per_vehicle_round_trip = rs5.number_input(
            "Vehicle Wear Fee Per Round-Trip Pair (JMD)",
            min_value=0.0,
            step=500.0,
        ...
        delivery_margin_percent = rs7.number_input(
            ...
            min_value=0.0,
            step=1.0,
            value=delivery_setting_float("delivery_margin_percent", 20.0),
            key="rate_delivery_margin_percent_input",
        )
        setup_hourly_rate = rs8.number_input(
            "Set-Up Labour Hourly Rate (JMD)",
            min_value=0.0,
            step=250.0,
    ...
    if delivery_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Delivery Calculator", expanded=True):
            ...
            trip_days = dcalc3.number_input(
                ...
                min_value=1,
                step=1,
                value=int(max(1, round_trip_count)),
            ...
            calculated_delivery_amount = round(delivery_subtotal_before_margin + margin_cost, 2)
    ...
    if setup_fee_mode == "Calculate Automatically":
        with st.expander("Automatic Set-Up Calculator", expanded=True):
            ...
            setup_hours = (total_setup_minutes * int(setup_crew_size)) / 60.0
            ...
            st.caption(
                f"Internal setup estimate -> Minutes: {total_setup_minutes:,.1f} | Crew-adjusted hours: {setup_hours:,.2f} | "
