...
def _build_invoice_image(payload: dict, logo_path: str | Path | None = None):
    ...
    def resolve_logo_candidates(candidate_path: str | Path | None) -> list[Path]:
        ...
        for assets_dir in assets_dirs:
            ...
            for pattern in ("*logo*.png", "*logo*.PNG", "*logo*.jpg", "*logo*.jpeg"):
                try:
    ...
    def font(size: int, bold: bool = False):
        streamlit_font_candidates: list[str] = []
        try:
            import streamlit  # type: ignore

            media_dir = (
                Path(streamlit.__file__).resolve().parent / "static" / "static" / "media"
            )
            if media_dir.exists():
                patterns = (
                ...
                for pattern in patterns:
                    streamlit_font_candidates.extend(
                        [str(path) for path in sorted(media_dir.glob(pattern))]
                    )
        except Exception:
            streamlit_font_candidates = []
