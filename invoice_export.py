...
def _build_invoice_image(payload: dict, logo_path: str | Path | None = None):
    ...
    def font(size: int, bold: bool = False):
        ...
        if font_candidates is None:
            candidates: list[str] = []
            # Prefer packaged fonts so all exports keep the same typography style.
            assets_dirs = [
                Path(__file__).resolve().with_name("assets"),
                Path.cwd() / "assets",
            ...
            # Streamlit ships KaTeX fonts. Find them robustly across versions/layouts.
            try:
            ...
            try:
                ...
                if pil_fonts.exists():
                    for pattern in (
                        ...
                        for hit in sorted(pil_fonts.glob(pattern[0])):
                            candidates.append(str(hit))
            except Exception:
                pass
