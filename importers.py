...
def _existing_invoice_event_date(invoice_number: str) -> str | None:
    number = _normalize_invoice_number(invoice_number)
    if not number:
        return None
    meta = invoice_meta_by_number(number)
    if not isinstance(meta, dict):
        return None
    return _date(meta.get("event_date", ""))


def _resolve_invoice_event_date(invoice_number: str, candidate_value: object) -> str | None:
    parsed = _date(candidate_value)
    if parsed:
        return parsed
    return _existing_invoice_event_date(invoice_number)
...
def import_net_profit_per_invoice(path: str) -> dict:
    df = _read_csv(path)
    counts = {"invoices": 0, "items": 0, "rows_seen": 0, "missing_event_date": 0}
    ...
    for _, row in df.iterrows():
        ...
        event_date = _resolve_invoice_event_date(invoice_number, row.get("Event Date", ""))
        if not event_date:
            counts["missing_event_date"] += 1
            # Confirmed invoices now require event date.
            continue
...
def import_expenses_per_invoice(path: str) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        event_date = _resolve_invoice_event_date(invoice_number, row.get("Event Date", ""))
        if not event_date:
            counts["skipped_missing_event_date"] += 1
            continue
...
def import_wages_per_person(path: str) -> dict:
    df = _read_csv(path)
    counts = {"invoices": 0, "expenses": 0, "rows_seen": 0, "skipped_missing_event_date": 0}
    ...
    for _, row in df.iterrows():
        ...
        expense_date = _resolve_invoice_event_date(invoice_number, row.get("Date", ""))
        if not expense_date:
            counts["skipped_missing_event_date"] += 1
            continue
...
def import_monthly_net_profit(path: str) -> dict:
    df = _read_csv(path)
    counts = {"inventory_purchases": 0, "rows_seen": 0}
    ...
    for _, row in df.iterrows():
        ...
        if purchase > 0:
            add_inventory_purchase(
                purchase_date=f"{month_key}-01",
                item_name="Imported Inventory Purchase",
                vendor="Monthly Net Profit CSV",
                quantity=0.0,
                amount=purchase,
                notes="Imported from Monthly Net Profit CSV",
            )
            counts["inventory_purchases"] += 1
    return counts
...
def import_shopify_orders(path: str) -> dict:
    ...
    df = _read_csv(path)
    counts = {"rows_seen": 0, "orders_imported": 0, "orders_skipped": 0, "skipped_missing_event_date": 0}
