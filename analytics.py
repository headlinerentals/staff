...
_EXPENSE_CATEGORY_ALIAS_MAP = {
    ...
    "rerental": "Re-Rental",
    "re rental": "Re-Rental",
    "supplier rental": "Re-Rental",
    "petrol": "Petrol",
    "fuel": "Petrol",
    "bad debt": "Bad Debt",
    "unforeseen expense": "Unforeseen Expense",
    "unforeseen": "Unforeseen Expense",
...
def normalize_expense_category(value: object) -> str:
    ...
    if not raw:
        return "Other"
    token = raw.lower()
    mapped = _EXPENSE_CATEGORY_ALIAS_MAP.get(token)
    if mapped:
        return mapped
    if raw in _CANONICAL_EXPENSE_CATEGORIES:
        return raw
    title = " ".join(word.capitalize() for word in raw.split())
    return title or "Other"


def canonical_expense_categories() -> list[str]:
    return list(_CANONICAL_EXPENSE_CATEGORIES)
...
def load_invoice_level() -> pd.DataFrame:
    ...
    linked_expenses = fetch_dataframe(
        ...
        WHERE invoice_id IS NOT NULL
          AND lower(COALESCE(expense_kind, 'transaction')) NOT IN ('summary_rollup', 'recurring_draft')
        GROUP BY invoice_id
        """
    )
...
def load_expenses() -> pd.DataFrame:
    ...
    expenses["category_raw"] = expenses.get("category", "").fillna("").astype(str)
    expenses["category"] = expenses["category_raw"].map(normalize_expense_category)
    ...
    if not expense_scope.empty:
        general_expenses = (
            expense_scope[
                ...
                & (~expense_scope["expense_kind"].str.lower().isin(["summary_rollup", "recurring_draft"]))
            ]
            .dropna(subset=["month"])
        ...
        if not scoped.empty:
            ...
            general_expenses = (
                scoped[
                    ...
                    & (~scoped["expense_kind"].str.lower().isin(["summary_rollup", "recurring_draft"]))
                ]
...
def load_weekly_summary() -> pd.DataFrame:
    ...
    scoped = daily.copy()
    scoped["day_ts"] = pd.to_datetime(scoped["day"], errors="coerce")
    scoped = scoped[scoped["day_ts"].notna()].copy()
    if scoped.empty:
        return pd.DataFrame(
            columns=[
                "week_start",
    ...
    scoped["week_start_ts"] = (
        scoped["day_ts"] - pd.to_timedelta(scoped["day_ts"].dt.weekday, unit="D")
    ).dt.normalize()
    weekly = (
        scoped.groupby("week_start_ts", as_index=False)[
            [
                "revenue",
                "cash_collected",
                "outstanding_receivables",
    ...
    weekly["week_end_ts"] = weekly["week_start_ts"] + pd.to_timedelta(6, unit="D")
    weekly["week_start"] = weekly["week_start_ts"].dt.strftime("%Y-%m-%d")
    weekly["week_end"] = weekly["week_end_ts"].dt.strftime("%Y-%m-%d")
    weekly["week_label"] = weekly["week_start"] + " to " + weekly["week_end"]
    weekly["month"] = weekly["week_start_ts"].dt.to_period("M").astype("string")
    weekly["year"] = weekly["week_start_ts"].dt.year
    return weekly[
        [
            "week_start",
            "week_end",
...
def load_expense_category_budget_vs_actual() -> pd.DataFrame:
    expenses = load_expenses()
    budgets = fetch_dataframe(
        """
        SELECT
    ...
    if expenses.empty and budgets.empty:
        return pd.DataFrame(
            columns=[
                "month",
                "month_label",
    ...
    actual = pd.DataFrame(columns=["month", "category", "amount_actual"])
    if not expenses.empty:
        scoped = expenses.copy()
        scoped = scoped[~_inventory_purchase_mask(scoped["category"])]
        scoped = scoped[
            ~scoped["expense_kind"].fillna("").astype(str).str.lower().isin(["summary_rollup", "recurring_draft"])
        ]
        ...
        if not scoped.empty:
            actual = (
                ...
                .sum()
                .rename(columns={"amount": "amount_actual"})
            )

    if not budgets.empty:
        budgets = budgets.copy()
        budgets["category"] = budgets["category"].map(normalize_expense_category)

    base = pd.DataFrame(columns=["month", "category"])
    if not actual.empty:
        base = pd.concat([base, actual[["month", "category"]]], ignore_index=True)
    if not budgets.empty:
        base = pd.concat([base, budgets[["month", "category"]]], ignore_index=True)
    base = base.dropna(subset=["month", "category"]).drop_duplicates()
    if base.empty:
        return pd.DataFrame(
            columns=[
                "month",
    ...
    out = (
        base.merge(actual, on=["month", "category"], how="left")
        .merge(budgets[["month", "category", "amount_target", "notes"]], on=["month", "category"], how="left")
    )
    out["month_label"] = pd.PeriodIndex(out["month"], freq="M").strftime("%b %Y")
    for col in ["amount_target", "amount_actual"]:
        out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0)
    out["variance"] = out["amount_target"] - out["amount_actual"]
    out["variance_pct"] = np.where(
        out["amount_target"] > 0,
        (out["variance"] / out["amount_target"]) * 100.0,
        0.0,
    )
    out["notes"] = out["notes"].fillna("")
    out = out.sort_values(["month", "category"])
    return out[
        [
            "month",
            "month_label",
...
def load_weekly_cashflow_forecast(weeks: int = 8, history_weeks: int = 8) -> pd.DataFrame:
    total_weeks = max(1, int(weeks or 8))
    lookback_weeks = max(1, int(history_weeks or 8))
    today = date.today()
    week_start = today - timedelta(days=today.weekday())

    invoice_level = load_invoice_level()
    expected_inflow = pd.DataFrame(columns=["week_start", "inflow_expected"])
    if not invoice_level.empty:
        invoice_scope = invoice_level.copy()
        invoice_scope["event_date"] = pd.to_datetime(invoice_scope["event_date"], errors="coerce")
        invoice_scope = invoice_scope[invoice_scope["event_date"].notna()].copy()
        invoice_scope = invoice_scope[invoice_scope["event_date"].dt.date >= week_start].copy()
        if not invoice_scope.empty:
            invoice_scope["week_start"] = (
                invoice_scope["event_date"]
                - pd.to_timedelta(invoice_scope["event_date"].dt.weekday, unit="D")
            ).dt.normalize()
            expected_inflow = (
                invoice_scope.groupby("week_start", as_index=False)["amount_outstanding"]
                .sum()
                .rename(columns={"amount_outstanding": "inflow_expected"})
            )

    expenses = load_expenses()
    avg_weekly_variable = 0.0
    if not expenses.empty:
        expense_scope = expenses.copy()
        expense_scope = expense_scope[~_inventory_purchase_mask(expense_scope["category"])]
        expense_scope = expense_scope[
            ~expense_scope["expense_kind"].fillna("").astype(str).str.lower().isin(["summary_rollup", "recurring_draft"])
        ]
        expense_scope["finance_date"] = pd.to_datetime(expense_scope["finance_date"], errors="coerce")
        expense_scope = expense_scope[expense_scope["finance_date"].notna()].copy()
        variable_scope = expense_scope[expense_scope["expense_kind"].str.lower() == "transaction"].copy()
        if not variable_scope.empty:
            lookback_start = pd.Timestamp(week_start - timedelta(weeks=lookback_weeks))
            lookback_end = pd.Timestamp(week_start)
            recent = variable_scope[
                (variable_scope["finance_date"] >= lookback_start)
                & (variable_scope["finance_date"] < lookback_end)
            ].copy()
            if not recent.empty:
                avg_weekly_variable = float(recent["amount"].sum()) / float(lookback_weeks)

    recurring_templates = fetch_dataframe(
        """
        SELECT
    ...
    for step in range(total_weeks):
        start_day = week_start + timedelta(weeks=step)
        end_day = start_day + timedelta(days=6)
        start_ts = pd.Timestamp(start_day)
        end_ts = pd.Timestamp(end_day)

        inflow = 0.0
        if not expected_inflow.empty:
            inflow = float(
                pd.to_numeric(
                    expected_inflow.loc[expected_inflow["week_start"] == start_ts, "inflow_expected"],
                    errors="coerce",
                ).fillna(0.0).sum()
            )

        recurring_due = 0.0
        if not recurring_templates.empty:
            covered_months = {(start_day.year, start_day.month), (end_day.year, end_day.month)}
            for _, tpl in recurring_templates.iterrows():
                amount = float(pd.to_numeric(tpl.get("default_amount", 0.0), errors="coerce") or 0.0)
                day_token = int(float(tpl.get("day_of_month", 1) or 1))
                if amount <= 0:
                    continue
                day_token = max(1, min(31, day_token))
                for year_token, month_token in covered_months:
                    last_dom = calendar.monthrange(int(year_token), int(month_token))[1]
                    due_dom = min(day_token, last_dom)
                    due_date = date(int(year_token), int(month_token), int(due_dom))
                    if start_day <= due_date <= end_day:
                        recurring_due += amount

        outflow_expected = float(avg_weekly_variable + recurring_due)
        net_expected = float(inflow - outflow_expected)
        running_net += net_expected

        forecast_rows.append(
            {
                "week_start": start_day.isoformat(),
                "week_end": end_day.isoformat(),
                "week_label": f"{start_day.isoformat()} to {end_day.isoformat()}",
                "inflow_expected": inflow,
                "outflow_expected": outflow_expected,
                "net_expected": net_expected,
                "cumulative_net": running_net,
                "avg_weekly_variable_expense": float(avg_weekly_variable),
                "recurring_due": float(recurring_due),
            }
        )
...
def load_finance_data_quality_checks() -> pd.DataFrame:
    expenses = load_expenses()
    invoice_level = load_invoice_level()
    ...
    if expenses.empty:
        return pd.DataFrame(
            columns=["issue_key", "issue", "count", "severity", "recommendation"]
        )
    ...
    missing_category = scoped[
        scoped["category"].fillna("").astype(str).str.strip().isin(["", "Other"])
    ]
    checks.append(
        {
            "issue_key": "missing_or_other_category",
            "issue": "Missing/Generic Category",
            "count": int(len(missing_category)),
            "severity": "medium",
            "recommendation": "Set a specific category instead of blank/Other.",
        }
    )

    critical_vendor_categories = {"wages", "re-rental", "google ads", "facebook ads", "chatgpt", "shopify"}
    missing_vendor = scoped[
        scoped["category"].fillna("").str.lower().isin(critical_vendor_categories)
        & (scoped["vendor"].fillna("").astype(str).str.strip() == "")
    ]
    checks.append(
        {
            "issue_key": "missing_vendor_critical_categories",
            "issue": "Missing Vendor/Person on Critical Categories",
            "count": int(len(missing_vendor)),
            "severity": "high",
            "recommendation": "Add vendor/person for wages, supplier, and ad/software spend.",
        }
    )

    non_standard_label = scoped[
        scoped["category_raw"].fillna("").astype(str).str.strip() != scoped["category"].fillna("").astype(str).str.strip()
    ]
    checks.append(
        {
            "issue_key": "non_standard_category_labels",
            "issue": "Non-Standard Category Labels",
            "count": int(len(non_standard_label)),
            "severity": "low",
            "recommendation": "Use standard category names to keep reports clean.",
        }
    )

    unlinked_transactions = scoped[
        (scoped["expense_kind"].fillna("").str.lower() == "transaction")
        & (scoped["invoice_id"].isna())
    ]
    checks.append(
        {
            "issue_key": "transaction_without_invoice_link",
            "issue": "Transaction Expense Without Invoice Link",
            "count": int(len(unlinked_transactions)),
            "severity": "high",
            "recommendation": "Link transaction expenses to confirmed invoices.",
        }
    )

    duplicate_rows = pd.DataFrame()
    if not scoped.empty:
        dup_base = scoped.copy()
        dup_base["finance_day"] = pd.to_datetime(dup_base["finance_date"], errors="coerce").dt.strftime("%Y-%m-%d")
        dup_base["amount_round"] = pd.to_numeric(dup_base["amount"], errors="coerce").fillna(0.0).round(2)
        dup_base["vendor_norm"] = dup_base["vendor"].fillna("").astype(str).str.strip().str.lower()
        dup_base["category_norm"] = dup_base["category"].fillna("").astype(str).str.strip().str.lower()
        dup_base = dup_base.dropna(subset=["finance_day"])
        if not dup_base.empty:
            dup_counts = (
                dup_base.groupby(["finance_day", "category_norm", "vendor_norm", "amount_round"], as_index=False)["id"]
                .count()
                .rename(columns={"id": "entry_count"})
            )
            duplicate_rows = dup_counts[dup_counts["entry_count"] > 1]
    checks.append(
        {
            "issue_key": "possible_duplicates",
            "issue": "Possible Duplicate Expense Rows",
            "count": int(
                duplicate_rows["entry_count"].sum()
                if ("entry_count" in duplicate_rows.columns and not duplicate_rows.empty)
                else 0
            ),
            "severity": "medium",
            "recommendation": "Review same-day same-category same-vendor same-amount rows.",
        }
    )

    missing_customer = invoice_level[
        invoice_level["customer_name"].fillna("").astype(str).str.strip() == ""
    ] if not invoice_level.empty else pd.DataFrame()
    checks.append(
        {
            "issue_key": "invoice_missing_customer_name",
            "issue": "Invoices Missing Customer Name",
            "count": int(len(missing_customer)),
            "severity": "low",
            "recommendation": "Add customer names for cleaner receivable and CRM tracking.",
        }
    )
    ...
    return out.sort_values(["count", "severity"], ascending=[False, True]).reset_index(drop=True)
...
def load_wages_by_person_monthly() -> pd.DataFrame:
    expenses = load_expenses()
    if expenses.empty:
        return pd.DataFrame(columns=["month", "month_label", "person", "amount", "entry_count"])
    ...
    out = (
        wages.groupby(["month", "vendor"], as_index=False)
        .agg(
            amount=("amount", "sum"),
            entry_count=("id", "count"),
        )
        .rename(columns={"vendor": "person"})
        .sort_values(["month", "amount"], ascending=[True, False])
    )
    out["month_label"] = pd.PeriodIndex(out["month"], freq="M").strftime("%b %Y")
    return out[["month", "month_label", "person", "amount", "entry_count"]]
...
def load_expense_breakdown_by_category() -> pd.DataFrame:
    ...
    expenses = expenses[
        ~expenses["expense_kind"].str.lower().isin(["summary_rollup", "recurring_draft"])
    ]
    if expenses.empty:
        return pd.DataFrame(columns=["category", "amount"])
    return (
