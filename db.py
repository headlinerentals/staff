...
REMOTE_BACKUP_STARTER_ROW_LIMIT = max(
    0,
    _config_int("HR_REMOTE_BACKUP_STARTER_ROW_LIMIT", 25),
)
...
def _starter_row_limit() -> int:
    try:
        return max(0, int(REMOTE_BACKUP_STARTER_ROW_LIMIT))
    except Exception:
        return 25
...
def _settings_state_for_path(db_path: Path | None) -> tuple[int, str]:
    """Returns (row_count, latest_updated_at) for app_settings at db_path.

    This exists because _operational_row_count_for_path deliberately only counts
    transactional tables (invoices/expenses/inventory/etc.) and was never intended
    to measure Rate Settings / Automatic Delivery Calculator configuration, which
    lives entirely in app_settings. Every backup/restore safety check in this file
    used to rely on the operational count alone, which meant a session that only
    changed Rate Settings (no new invoices) was invisible to those checks."""
    target = Path(db_path) if db_path else Path("")
    if not target.exists() or not target.is_file():
        return (0, "")
    try:
        with sqlite3.connect(target) as conn:
            row = conn.execute(
                "SELECT COUNT(*), COALESCE(MAX(updated_at), '') FROM app_settings"
            ).fetchone()
            if row:
...
def _has_meaningful_local_state(db_path: Path | None) -> bool:
    """True if db_path has real transactional rows OR any saved app_settings.

    A genuinely fresh/starter Streamlit runtime has zero of both. The moment a
    user saves even one Rate Setting, this must be True — settings are real,
    deliberate work and must never again be treated as disposable."""
    operational_rows = int(_operational_row_count_for_path(db_path))
    settings_rows, _ = _settings_state_for_path(db_path)
    return operational_rows > 0 or settings_rows > 0


def _is_still_starter_state(db_path: Path | None) -> bool:
    """True only if BOTH operational rows and app_settings are empty/trivial.

    Used to gate the safety bypasses that treat a database as disposable. Those
    bypasses must never fire once real settings have been saved, even if
    invoice/expense/inventory row counts are still low or zero — which is the
    normal, expected state for a settings-heavy user."""
    operational_rows = int(_operational_row_count_for_path(db_path))
    settings_rows, _ = _settings_state_for_path(db_path)
    return operational_rows <= _starter_row_limit() and settings_rows <= 0


def _local_state_wins_or_ties(local_path: Path | None, other_path: Path | None) -> bool:
    """True if local_path's state is at least as rich as other_path's, on EITHER
    the transactional-row dimension OR the settings-freshness dimension.

    This is the core fix: previously, only the operational row count was
    compared, so a session that only updated Rate Settings (same or fewer
    invoices than before) could never win this comparison against an older
    backup that happened to have more invoices — even though the settings
    were genuinely newer and more complete. Now either dimension winning is
    enough, matching how the app is actually used."""
    local_operational = int(_operational_row_count_for_path(local_path))
    other_operational = int(_operational_row_count_for_path(other_path))
    if local_operational >= other_operational:
        return True
    local_settings_rows, local_settings_at = _settings_state_for_path(local_path)
    _, other_settings_at = _settings_state_for_path(other_path)
    if local_settings_rows > 0 and local_settings_at >= other_settings_at:
        return True
    return False
...
_LAST_REMOTE_PUSH_INFO: dict[str, object] = {}


def _record_push_result(ok: bool, reason: str) -> bool:
    global _LAST_REMOTE_PUSH_INFO
    _LAST_REMOTE_PUSH_INFO = {
        "ok": bool(ok),
        "reason": str(reason),
...
def get_last_remote_push_info() -> dict[str, object]:
    if not isinstance(_LAST_REMOTE_PUSH_INFO, dict):
        return {}
    return dict(_LAST_REMOTE_PUSH_INFO)
...
def push_remote_backup_snapshot(reason: str = "auto", force: bool = False) -> bool:
    ...
    if not REMOTE_BACKUP_ENABLED:
        return _record_push_result(
            False,
            "Remote backup is not enabled — HR_REMOTE_BACKUP_REPO or "
    ...
    if not _has_meaningful_local_state(DB_PATH):
        return _record_push_result(
            False, "Local database has no operational rows or saved settings yet."
        )
    if _is_still_starter_state(DB_PATH) and not bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH):
        # Do not let Streamlit's fresh/default DB overwrite the last real backup.
        # _is_still_starter_state checks app_settings too, so this no longer fires
        # once real Rate Settings have been saved, even with few/no invoices.
        return _record_push_result(
            False, "Local database still looks like a fresh/starter state."
        )
    ...
    if not force and (now_epoch - float(_LAST_REMOTE_PUSH_AT)) < float(REMOTE_BACKUP_MIN_SECONDS):
        return _record_push_result(
            False,
            f"Skipped — pushed less than {int(REMOTE_BACKUP_MIN_SECONDS)}s ago; "
            "use force=True to override this rate limit.",
        )

    try:
        payload_bytes = DB_PATH.read_bytes()
    except Exception as exc:
        return _record_push_result(False, f"Could not read the local database file: {exc}")
    if not payload_bytes:
        return _record_push_result(False, "Local database file is empty.")

    status_get, meta = _github_json_request(_github_contents_api_url(with_ref=True), method="GET")
    if status_get not in {200, 404}:
        api_message = str(meta.get("message") or "").strip() if isinstance(meta, dict) else ""
        return _record_push_result(
            False,
            f"GitHub rejected the check for the existing backup file (HTTP {status_get})."
            + (f" GitHub said: {api_message}" if api_message else " No response body — likely a network/DNS problem."),
        )
    ...
    if status_get == 200 and not bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH):
        remote_tmp_path = DB_PATH.parent / f".{DB_PATH.name}.remote_compare.tmp"
        try:
            remote_bytes_for_compare = _download_remote_backup_bytes()
            if remote_bytes_for_compare:
                remote_tmp_path.write_bytes(remote_bytes_for_compare)
                if not _local_state_wins_or_ties(DB_PATH, remote_tmp_path):
                    # Remote genuinely has more operational rows AND local settings
                    # are not newer — real reason to hold off, not a settings blind spot.
                    return _record_push_result(
                        False,
                        "Held back on purpose: the remote backup currently has more "
                        "operational rows AND local settings are not newer than "
                        "remote settings. If this looks wrong, that's worth checking "
                        "directly rather than trusting this message alone.",
                    )
        finally:
            try:
    ...
    status_put, put_response = _github_json_request(
    ...
    if ok:
        ...
        return _record_push_result(True, f"Pushed successfully (HTTP {status_put}).")
    api_message = str(put_response.get("message") or "").strip() if isinstance(put_response, dict) else ""
    return _record_push_result(
        False,
        f"GitHub rejected the backup commit (HTTP {status_put})."
        + (f" GitHub said: {api_message}" if api_message else " No response body — likely a network/DNS problem."),
    )
...
def restore_remote_backup_if_needed() -> bool:
    ...
    starter_limit = _starter_row_limit()
    local_is_starter = _is_still_starter_state(DB_PATH)
    if _is_auto_restore_suppressed() and not local_is_starter:
        return False
    ...
    try:
        ...
        remote_settings_rows, _ = _settings_state_for_path(tmp_path)
        if remote_rows <= 0 and remote_settings_rows <= 0:
            tmp_path.unlink(missing_ok=True)
            return False

        # starter_restore only applies when local is a genuinely fresh runtime —
        # zero operational rows AND zero saved app_settings. The moment even one
        # Rate Setting has been saved locally, local_is_starter is False and this
        # bypass can never fire again, regardless of invoice/expense count.
        starter_restore = local_is_starter and not _local_state_wins_or_ties(DB_PATH, tmp_path)

        # Safety: never overwrite meaningful local data unless explicitly allowed.
        if not local_is_starter and not bool(REMOTE_BACKUP_ALLOW_NONEMPTY_RESTORE) and not starter_restore:
            tmp_path.unlink(missing_ok=True)
            return False
        if restore_mode == "empty_only" and not local_is_starter and not starter_restore:
            tmp_path.unlink(missing_ok=True)
            return False

        if (
            restore_mode == "empty_or_smaller"
            and not local_is_starter
            and _local_state_wins_or_ties(DB_PATH, tmp_path)
        ):
            tmp_path.unlink(missing_ok=True)
            return False
...
def create_db_backup_snapshot(reason: str = "auto", force: bool = False) -> Path | None:
    ...
    if _is_still_starter_state(DB_PATH) and not bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH):
        # Do not bless a newly reset/default database as the latest good backup.
        # _is_still_starter_state also checks app_settings, so a database with
        # real saved Rate Settings is never treated as "newly reset" here.
        return latest_auto_backup_path()
    ...
    try:
        ...
        _record_push_result(False, f"Push raised an unexpected error: {exc}")
    return backup_path
...
def restore_latest_backup_if_empty() -> bool:
    ...
    if _is_auto_restore_suppressed() and not local_is_starter:
        return False
    if not local_is_starter:
        # Real Rate Settings (or real invoices) exist locally — never silently
        # replace them from an older local snapshot just because the operational
        # row count happens to be low.
        return False

    latest = latest_auto_backup_path()
    if latest is None or not latest.exists():
        return False
    if _local_state_wins_or_ties(DB_PATH, latest):
        return False
...
def db_storage_status() -> dict[str, object]:
    ...
    return {
        ...
        "db_settings_rows": _settings_state_for_path(DB_PATH)[0],
        "db_settings_last_updated": _settings_state_for_path(DB_PATH)[1],
        "latest_backup_settings_rows": _settings_state_for_path(latest)[0] if latest else 0,
        ...
        "remote_backup_starter_row_limit": int(_starter_row_limit()),
        "remote_backup_allow_smaller_push": bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH),
...
def restore_db_from_snapshot(snapshot_path: str) -> dict[str, object]:
    ...
    if not _has_meaningful_local_state(target):
        raise ValueError("Selected backup has no operational rows or saved settings.")
...
def restore_db_from_uploaded_bytes(file_bytes: bytes, original_name: str = "uploaded_backup.db") -> dict[str, object]:
    ...
    try:
        ...
        if not _has_meaningful_local_state(tmp_path):
            raise ValueError("Uploaded backup has no operational rows or saved settings.")
...
def init_db() -> None:
    ...
    rows_before_restore = int(_operational_row_count_for_path(DB_PATH))
    should_try_restore = (
        not _is_auto_restore_suppressed()
        or rows_before_restore <= _starter_row_limit()
    )
...
def export_delivery_settings_json() -> str:
    """Exports every Rate Settings value AND the four reference tables (Setup
    Minutes, Public Holidays, Route Toll Presets, East-West Toll) as one JSON
    file. All of these are stored under the 'delivery.' key prefix in
    app_settings, including the table JSON blobs, so this single prefix query
    captures everything without needing to know every individual field name —
    new Rate Settings fields added later are automatically included too."""
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT setting_key, setting_value FROM app_settings WHERE setting_key LIKE 'delivery.%'"
        ).fetchall()
    settings = {str(row["setting_key"]): str(row["setting_value"]) for row in rows}
    payload = {
...
def import_delivery_settings_json(json_text: str) -> int:
    """Restores Rate Settings + reference tables from a JSON export produced by
    export_delivery_settings_json(). Only keys under the 'delivery.' prefix are
    accepted, so an unrelated or malformed file cannot write into other parts
    of app_settings. Returns the number of keys restored."""
    ...
    if not isinstance(settings, dict) or not settings:
        raise ValueError("Backup file has no recognizable Rate Settings content.")
    restored = 0
    for key, value in settings.items():
        if isinstance(key, str) and key.startswith("delivery."):
            set_setting(key, str(value))
            restored += 1
    if restored <= 0:
        raise ValueError("Backup file contained no 'delivery.*' settings to restore.")
    return restored
