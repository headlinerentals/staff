...
def push_remote_backup_snapshot(reason: str = "auto", force: bool = False) -> bool:
    ...
    if local_rows <= 0:
        return False
    if local_rows <= _starter_row_limit() and not bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH):
...
def restore_remote_backup_if_needed() -> bool:
    ...
    if _is_auto_restore_suppressed() and local_rows > starter_limit:
        return False
...
def create_db_backup_snapshot(reason: str = "auto", force: bool = False) -> Path | None:
    if not DB_PATH.exists() or not DB_PATH.is_file():
        return None
    current_rows = int(_operational_row_count_for_path(DB_PATH))
    if current_rows <= 0:
        return latest_auto_backup_path()
    if current_rows <= _starter_row_limit() and not bool(REMOTE_BACKUP_ALLOW_SMALLER_PUSH):
...
def restore_latest_backup_if_empty() -> bool:
    ...
    if _is_auto_restore_suppressed() and local_rows > starter_limit:
        return False
    if local_rows > starter_limit:
        return False

    latest = latest_auto_backup_path()
    if latest is None or not latest.exists():
        return False
    latest_rows = int(_operational_row_count_for_path(latest))
    if latest_rows <= local_rows:
        return False
