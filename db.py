...
AUTO_BACKUP_MIN_SECONDS = max(
    15,
    int(float((os.getenv("HR_AUTO_BACKUP_MIN_SECONDS", "30") or "30").strip() or 30)),
)
AUTO_BACKUP_KEEP = max(
    5,
    int(float((os.getenv("HR_AUTO_BACKUP_KEEP", "120") or "120").strip() or 120)),
)
...
def _resolve_db_mirror_paths() -> list[Path]:
    ...
    """
    configured = (os.getenv("HR_DB_MIRROR_PATHS", "") or "").strip()
...
REMOTE_BACKUP_REPO = (os.getenv("HR_REMOTE_BACKUP_REPO", "") or "").strip()
REMOTE_BACKUP_TOKEN = (os.getenv("HR_REMOTE_BACKUP_TOKEN", "") or "").strip()
REMOTE_BACKUP_BRANCH = (os.getenv("HR_REMOTE_BACKUP_BRANCH", "main") or "main").strip()
REMOTE_BACKUP_FILE = (
    os.getenv("HR_REMOTE_BACKUP_FILE", "backups/finance_hub_live_backup.db")
    or "backups/finance_hub_live_backup.db"
).strip()
REMOTE_BACKUP_MIN_SECONDS = max(
    30,
    int(float((os.getenv("HR_REMOTE_BACKUP_MIN_SECONDS", "120") or "120").strip() or 120)),
)
REMOTE_BACKUP_PULL_ON_INIT = str(os.getenv("HR_REMOTE_BACKUP_PULL_ON_INIT", "1")).strip().lower() in {
    "1",
    "true",
    "yes",
}
REMOTE_BACKUP_ENABLED = bool(REMOTE_BACKUP_REPO and REMOTE_BACKUP_TOKEN)
...
def _operational_row_count_for_path(db_path: Path | None) -> int:
    ...
    if not target.exists() or not target.is_file():
        return 0
    tables = ("invoices", "invoice_items", "expenses", "inventory_items", "inventory_movements")
    total = 0
    try:
...
def restore_remote_backup_if_needed() -> bool:
    ...
    - `empty_or_smaller` (restore if local row count is smaller than remote)
    """
    if not REMOTE_BACKUP_ENABLED or not REMOTE_BACKUP_PULL_ON_INIT:
        return False

    restore_mode = (
        str(os.getenv("HR_REMOTE_BACKUP_RESTORE_MODE", "empty_or_smaller")).strip().lower()
        or "empty_or_smaller"
    )
    if restore_mode not in {"empty_only", "empty_or_smaller"}:
        restore_mode = "empty_or_smaller"

    local_rows = _operational_row_count_for_path(DB_PATH)
    if restore_mode == "empty_only" and local_rows > 0:
        return False
...
def db_storage_status() -> dict[str, object]:
    ...
    return {
        ...
        "remote_backup_restore_mode": str(
            os.getenv("HR_REMOTE_BACKUP_RESTORE_MODE", "empty_or_smaller") or "empty_or_smaller"
        ).strip().lower(),
    }
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            ...
            INSERT INTO invoices (
                ...
                delivered_to, paid_to, payment_status, amount_paid, deposit_balance_enabled, payment_notes, notes
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    with get_connection() as conn:
        ...
        for row in templates:
            ...
            existing_expense = conn.execute(
                ...
                WHERE date(expense_date) = date(?)
                  ...
                  AND lower(COALESCE(expense_kind, 'transaction')) = 'recurring_monthly'
                  AND lower(COALESCE(vendor, '')) = lower(?)
                  AND lower(COALESCE(description, '')) = lower(?)
                  AND ABS(amount - ?) < 0.0001
                ORDER BY id DESC
        ...
        if posted_count > 0:
            conn.execute(
                ...
                """,
                (f"{month_token}: posted {posted_count} recurring expense(s).",),
            )
