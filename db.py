...
def _resolve_db_path() -> Path:
    ...
    try:
        ...
        db_path.parent.mkdir(parents=True, exist_ok=True)
    if db_path != legacy_local and not db_path.exists() and legacy_local.exists():
        # One-time migration to persistent storage when app files are moved/reuploaded.
        try:
...
AUTO_BACKUP_MIN_SECONDS = max(
    15,
    int(float((os.getenv("HR_AUTO_BACKUP_MIN_SECONDS", "120") or "120").strip() or 120)),
)
